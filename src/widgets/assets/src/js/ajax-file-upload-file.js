...
(function(sx, $, _)
{
    ...
    sx.classes.fileupload._File = sx.classes.Component.extend({
        ...
        render: function()
        {
            ...
            this.JFormData       = $('<div>', {'class' : 'form-data'});
            ...
            defaultOptions = this.Uploader.get('defaultOptions');
            additionalFields = defaultOptions.additionalFields;
            if(self.get('additionalFields'))
                additionalFields = self.get('additionalFields');

            if(additionalFields) {
                $.each(additionalFields, function (key, field) {
                    inputName = 'CmsContentElement[uploadData][' + self.getValue() + '][' + key + ']';
                    var row  = self.JFormData.append('<div>',{'class':'form-group'});
                    row.append($('<label>', {'class': 'control-label'}).text(field.label));
                    row.append($('<input>', {'name': inputName, 'class': 'form-control'}).val(field.value));
                });
            }

            this.JThumbWrapper.append(this.JFilePrev).append(this.JCaption).append(this.JFormData);
