...
class HasStorageFileMulti extends Behavior
{
                ...
                foreach ($this->owner->{$fieldCode} as $fileId) {
                    if (is_string($fileId) && ((string)(int)$fileId != (string)$fileId)) {
                        try {
                            ...
                            if (gettype($additionalData = @$form['uploadData'][$fileId]) == 'array') {
    ...
    public function afterSaveStorgaFile()
    {
        ...
        /*
         * обновить атрибуты дополнительные
         */
        ...
        foreach ($this->relations as $data) {
            ...
            if ($this->owner->{$relation}) {
                foreach ($this->owner->{$relation} as $storageFile) {
                    if ($additionalAttributes = @$form['uploadData'][$storageFile->primaryKey]) {
